...
class MySQLUserRepository(IUserRepository):
    ...
    async def get_user_by_id(self, user_id: int) -> Optional[User]:
        ...
        cursor = conn.cursor(dictionary=True)
        try:
            cursor.execute("SELECT id, username, email, password_hash, role_id, kea_client_id, unit_id, is_active, created_at, updated_at FROM users WHERE id = %s", (user_id,))
            result = cursor.fetchone()
            return User(**result) if result else None
        finally:
            cursor.close()
        ...
        try:
            query = """
                INSERT INTO transactions (cash_register_id, transaction_type, amount, description, 
                                        transaction_date, category, payment_method, unit_id, kea_client_id, role_id)
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
            """
            values = (
                ...
                transacao.payment_method, transacao.unit_id, transacao.kea_client_id, transacao.role_id
            )
            cursor.execute(query, values)
            conn.commit()
    ...
    async def update_transacao(self, transacao_id: int, transacao: Transacao) -> Optional[Transacao]:
        ...
        try:
            query = """
                ...
                amount = %s, transaction_date = %s, payment_method = %s, unit_id = %s, kea_client_id = %s, role_id = %s
                WHERE id = %s
            """
            values = (
                ...
                transacao.unit_id, transacao.kea_client_id, transacao.role_id, transacao_id
            )
            cursor.execute(query, values)
            conn.commit()
